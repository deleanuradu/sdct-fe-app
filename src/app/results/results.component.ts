...
export class ResultsComponent implements OnInit {
  ...
  results = {
    sysAnalyst: 0, softwareDev: 0, webMultimedia: 0, appProgrammer: 0,
  }
  ...
  private computeResults(): void {
    this.results.sysAnalyst = +((this.sysAnalystScore - this.min1) / (this.max1 - this.min1) * 100).toFixed(2);
    this.results.softwareDev = +((this.softwareDevScore - this.min2) / (this.max2 - this.min2) * 100).toFixed(2);
    this.results.webMultimedia = +((this.webMultimediaScore - this.min3) / (this.max3 - this.min3) * 100).toFixed(2);
    this.results.appProgrammer = +((this.appProgrammerScore - this.min4) / (this.max4 -  - this.min4) * 100).toFixed(2);
  }
}
